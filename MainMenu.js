class MainMenu extends Phaser.Scene {
    ...
    create () {
        this.navigating=false; this.bookNode=null;
        const progress = window.ACTIVITIES[0]?.v6 ? window.JourneySystem.ensure() : window.PersistenceService.load();
        ...
        if (progress.journey?.pendingReturn) { this.bookNode=host; window.JourneySystem.showDeath(this); return; }
        if (progress.journey?.completed) { this.bookNode=host; window.JourneySystem.showFinal(this); window.AudioSystem?.mount(this, screen.querySelector('.menu-card')); this.events.once('shutdown',()=>{host.textContent='';}); return; }
        ...
        const current = window.ACTIVITIES[Math.max(0, (progress.journey?.currentActivity || progress.unlockedMax) - 1)];
