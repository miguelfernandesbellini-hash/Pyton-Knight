...
| `tests/run-tests.cjs` | Suíte original, preservada byte a byte |
...
As pastas `02_DOCUMENTACAO_OFICIAL`, `03_ASSETS_VISUAL_OFICIAL` e `04_RELATORIOS_E_CONTROLE` não foram editadas. Relatórios antigos são históricos: os documentos em `docs/` descrevem esta atualização. Consulte o manifesto para cada arquivo criado/alterado; nenhum arquivo recebido foi removido ou renomeado.
